package com.moucan.customnetwork.widget;
...
public class FoldTextView extends AppCompatTextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isOverMaxLine && !isExpand) {
            //折叠
            if (mTipGravity == END) {
                minX = getWidth()  - getPaddingRight() - getTextWidth(mFoldText);
    ...
    private boolean isInRange(float x, float y) {
        if (minX < maxX) {
            return x >= minX  && x <= maxX + getPaddingLeft() + getPaddingRight() && y >= minY && y <= maxY;
        } else {
            return x <= maxX  && y >= middleY && y <= maxY || x >= minX+getPaddingLeft() + getPaddingRight() && y >= minY && y <= middleY;
        }
    }
